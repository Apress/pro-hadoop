...
import org.apache.hadoop.mapred.lib.CopyOfKeyFieldHelper.KeyDescription;
...
public class Copy_2_of_KeyFieldBasedComparator<K, V> extends WritableComparator 
implements JobConfigurable {
  private CopyOfKeyFieldHelper copyOfKeyFieldHelper = new CopyOfKeyFieldHelper();
  ...
  public void configure(JobConf job) {
    ...
    copyOfKeyFieldHelper.setKeyFieldSeparator(keyFieldSeparator);
    copyOfKeyFieldHelper.parseOption(option);
  }
  
  public Copy_2_of_KeyFieldBasedComparator() {
    super(Text.class);
  }
  ...
  public int compare(byte[] b1, int s1, int l1,
    ...
    List <KeyDescription> allKeySpecs = copyOfKeyFieldHelper.keySpecs();
    if (allKeySpecs.size() == 0) {
      return compareBytes(b1, s1+n1, l1-n1, b2, s2+n2, l2-n2);
    }
    int []lengthIndicesFirst = copyOfKeyFieldHelper.getWordLengths(b1, s1+n1, s1+l1);
    int []lengthIndicesSecond = copyOfKeyFieldHelper.getWordLengths(b2, s2+n2, s2+l2);
    for (KeyDescription keySpec : allKeySpecs) {
      int startCharFirst = copyOfKeyFieldHelper.getStartOffset(b1, s1+n1, s1+l1, lengthIndicesFirst,
          keySpec);
      int endCharFirst = copyOfKeyFieldHelper.getEndOffset(b1, s1+n1, s1+l1, lengthIndicesFirst,
          keySpec);
      int startCharSecond = copyOfKeyFieldHelper.getStartOffset(b2, s2+n2, s2+l2, lengthIndicesSecond,
          keySpec);
      int endCharSecond = copyOfKeyFieldHelper.getEndOffset(b2, s2+n2, s2+l2, lengthIndicesSecond,
          keySpec);
      int result;
  ...
  protected int compareByteSequence(byte[] first, int start1, int end1, 
	  ...
	  KeyDescription key = (KeyDescription) keyObj;
    if (start1 == -1) {
      if (key.reverse) {
        return 1;
      }
  ...
  public int numericalCompare (byte[] a, int start1, int end1, 
